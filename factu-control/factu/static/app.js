...
const batchSelect=document.getElementById('batch-select');if(batchSelect)batchSelect.addEventListener('change',()=>batchSelect.form.submit());
...
function runMessage(r){if(!r.run.active)return r.run.error||'Trabajo completado.';if(r.run.task==='ERP'&&r.run.erp){const e=r.run.erp;if(e.page&&e.pages)return `Sincronizando ERP: página ${e.page} de ${e.pages}${e.retries?` · ${e.retries} reintento${e.retries===1?'':'s'}`:''}.`;return 'Sincronizando ERP: iniciando sesión y pidiendo la primera página…';}return `${r.run.task}: factura ${r.metrics.decisions} de ${r.metrics.documents} · quedan ${r.metrics.pending}.`;}
let pollTimer;async function poll(batch){clearTimeout(pollTimer);try{const response=await fetch(`/api/batches/${batch}/status`);if(!response.ok)throw Error('estado');const r=await response.json();working(!!r.run.active);const status=document.getElementById('run-status');if(status){status.hidden=false;status.textContent=runMessage(r);}if(r.run.error){toast(r.run.error);return;}if(r.run.active)pollTimer=setTimeout(()=>poll(batch),1800);else location.reload();}catch(e){working(false);toast('No se puede consultar el estado. Los trabajos conservan su último checkpoint.');}}
