...
Método `modelo` (nuevo): tercer paso de lectura para páginas escaneadas cuyos campos el OCR local no resuelve. Un modelo multimodal **solo lee** valores con su evidencia literal; nunca decide, y cada lectura pasa una validación propia antes de aceptarse (ver «Lectura con modelo»). Cualquier fallo del proveedor deja el aviso `MODELO_NO_DISPONIBLE` y la factura en ESCALAR. 145 pruebas Python + 4 JavaScript pasan.
...
Failover medido, no prometido: sin credenciales no se toca la red y cada escaneado sin resolver queda en ESCALAR con el aviso `MODELO_NO_DISPONIBLE: sin_clave` (lote 1 completo en 44 s en un MacBook Air M1, ERP con latencia real). Timeouts, 429 y 5xx tienen reintentos acotados; cualquier fallo final es un aviso en la extracción, nunca una excepción ni un resultado inventado. La caché de extracción incluye backend, modelo y disponibilidad de credenciales: al recuperarlas, `reextract` + `process` reextrae el lote (los PDF con texto tardan segundos y no llaman al modelo) y solo los escaneados pendientes consultan el modelo.

Coste: cada llamada queda registrada por documento en `costs` (etapa `modelo`) con tokens, neuronas de Workers AI y `external_eur` calculado con el precio configurado. En el ensayo previo sobre 26 escaneados (Workers AI, `@cf/meta/llama-4-scout-17b-16e-instruct`): ≈ 2.400 tokens de entrada y ≈ 67 neuronas por página, ≈ 0,0007 $ por página al precio publicado de 0,011 $/1.000 neuronas, dentro de la cuota gratuita de 10.000 neuronas/día. Pendiente de repetir la medición completa desde la app y actualizar estas cifras.
